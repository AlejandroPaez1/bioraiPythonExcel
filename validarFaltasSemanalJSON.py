...
def buscar_no_checadores(archivo_checadores):
    with open(archivo_checadores, 'r', encoding='utf-8') as file:
        data_checadores = json.load(file)

    # Crear un diccionario para almacenar las fechas de las checadas por PIN
    checadas_por_pin = defaultdict(list)
    for empleado in data_checadores["data"]:
        pin = empleado["PIN"]
...
# Solicitar al usuario que seleccione el archivo JSON
archivo_checadores = filedialog.askopenfilename(title="Seleccione el archivo JSON")
